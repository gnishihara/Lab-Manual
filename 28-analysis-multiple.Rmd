...
尤度 $(\mathcal{L})$ を求めるためには，誤差項の確率分布を指定する必要があります。ベクトル $x$ の平均値は `r mean(c(5, 3, 6, 3, 6))` です。
...
```
```{r, echo = FALSE}
out = bbmle::mle2(nll, data = x, start = list(mean = 5, sd = 1))
lik = as.numeric(bbmle::logLik(out))
aicv = AIC(out)
```

対数尤度は `r round((lik),4)`，尤度は `r round(exp(lik),4)` です。

このときのAICは 
$$
AIC = -2\times `r round((lik), 2)` + 2\times2=`r round(aicv, 2)`
$$
