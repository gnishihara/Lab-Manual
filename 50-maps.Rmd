...
```{r, echo = FALSE}
pout = pmain + (pomura / pkatagami)
proot = rprojroot::find_rstudio_root_file("Images/")
...
image_read_pdf(str_c(proot, pdfname), density = 600) |> 
  image_trim() |> image_border(color = "white") |> image_write(str_c(proot, pdfname))
```

```{r final-figure, echo = FALSE}
knitr::include_graphics(str_c(proot, pngname))
```
